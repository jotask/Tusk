...
public class Lantern implements Disposable{
    ...
    public Lantern(Player player) {
        ...
        light = new ConeLight(rayHandler, 500, Color.WHITE, 10f, player.getPosition().x, player.getPosition().y, angle, coneDegree);
        light.setActive(false);
    }

    public void update(){
        if(player.getController().light()) light.setActive(!light.isActive());
        light.setPosition(player.getBody().getPosition());
        this.angle = player.getAngleFromThis(Game.getInstance().getCamera().getMousePosInGameWorld());
